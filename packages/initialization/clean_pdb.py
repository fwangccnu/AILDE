...
def clean(complex,ligand_name):
	...
	for line in f13.readlines():                                           #Rewrite the ligand's atom name
	        if line[17:20]==ligand_name:
	                CB = line[12:16].strip()[:2]
	                if CB.lower() == 'cl' and line[76:78].strip().lower() == 'cl':
	                        line = line.replace(line[12:14],'Cl',1)
	                elif CB.lower() == 'br' and line[76:78].strip().lower() == 'br':
	                        line = line.replace(line[12:14],'Br',1)
	                else:
	                        pass
